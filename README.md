...
same 2-hourly job (see `src/build_dashboard.py`). This repo is **public** (required for
GitHub Pages on the free plan) — the two API keys only ever exist as encrypted GitHub
Actions secrets and a local, gitignored `.env`; they were never committed.
...
- `src/build_dashboard.py` — reads `data/stocks/` + `data/thesis/`, computes each stock's
  ...
  template, writes `docs/index.html`. Runs every 2 hours as part of the workflow below.
